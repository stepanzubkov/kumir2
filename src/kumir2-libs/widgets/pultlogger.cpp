...
int logLine::pos() const
{
	return textLabel->y();
}

void logLine::removeLabels()
{
	if (textLabel) {
		delete textLabel;
	}
...
loggerButton::loggerButton(const QDir &dir, QWidget *parent): QWidget(parent)
{
	isUpArrow = false;
	posX = 1;
	posY = 1;
...
void loggerButton::loadButtons(const QDir &dir)
{
...
pultLogger::pultLogger(const QDir &dir, QWidget *parent): QWidget(parent)
{
	...
	downButton = new loggerButton(dir, parent);
	downButton->move(0, H - 24);
	downButton->resize(140, 24);
	downButton->show();

	upButton = new loggerButton(dir, parent);
	upButton->move(0, 8);
...
void pultLogger::CopyLog()
{
	...
	QString text;
	for (int i = 0; i < lines.count(); i++)
		if (!lines[i].KumCommand().isEmpty()) {
			text += lines[i].KumCommand() + "\n";
		}
	cp->setText(text);
}


void pultLogger::Show()
{
	//show();
}
...
QString pultLogger::log()
{
	QString res;
	for (int i = 0; i < lines.count(); i++) {
		res += lines[i].KumCommand();
	}
	return res;
}
...
MainButton::MainButton(const QDir &dir, QWidget *parent): QWidget(parent)
{
	direction = UP;
	posX = 1;
	posY = 1;
...
void MainButton::paintEvent(QPaintEvent *event)
{
	...
	if (!downFlag) {
		painter.drawImage(QPoint(posX, posY), buttonImageUp);
		if (icon) {
			painter.drawImage(posX + iconoffs, posX + iconoffs, buttonIcon);
		}
...
void MainButton::drawAddons(QPainter *painter)
{
	...
	if (direction == LEFT) {
		painter->drawLines(leftArrow);
	}
	if (direction == RIGHT) {
		painter->drawLines(rightArrow);
	}
	if (direction == 5) {
		if (!downFlag) {
		...
		for (int i = 0; i < textLines.count(); i++) {
			painter->drawText(7, start_pos + TEXT_STEP * i, textLines[i]);
		}
	}
}
