...
private:
	 ...
	 * @return Fixed string
	 */
	static std::string fix_corrupted_data(const std::string &in);
	 ...
	 */
	static std::string preprocess_input(std::istream &stream);
	...
	static void read_command(const std::string &name, command_t &command);
	...
	static bytes decode_base64(const std::string &src);
