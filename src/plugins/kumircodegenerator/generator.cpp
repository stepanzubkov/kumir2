...
)
{
	...
	case Bytecode::VT_int:
		result = VM::AnyValue(val.toInt());
		break;
	case Bytecode::VT_real:
		result = VM::AnyValue(val.toDouble());
		break;
	case Bytecode::VT_bool:
		result = VM::AnyValue(bool(val.toBool()));
		break;
	case Bytecode::VT_char:
		result = VM::AnyValue(Kumir::Char(val.toChar().unicode()));
		break;
	case Bytecode::VT_string:
		result = VM::AnyValue(val.toString().toStdWString());
		break;
...
void Generator::setBreakAddress(
	QList<Bytecode::Instruction> &instrs,
	int level,
	int address
) {
